...
public class RemedyHelper extends EntityHelper {
	...
	public String getSpecificRemedy(String id) {
		...
		Property hasPlantPartUsage = getOntModel().getProperty(OntologyUtils.NS + "hasPlantPartUsage");
		Property hasAdjuvantId = getOntModel().getProperty(OntologyUtils.NS + "has_adjuvant_id");
		Property hasTherapeuticalId = getOntModel().getProperty(OntologyUtils.NS + "has_disease_id");
		Property hasMedicalCondition = getOntModel().getProperty(OntologyUtils.NS + "has_medical_condition");
		Property hasMedicalConditionId = getOntModel().getProperty(OntologyUtils.NS + "has_medical_condition_id");
		Property hasMedicalConditionMinAge = getOntModel().getProperty(OntologyUtils.NS + "has_min_age");

		Property isAffectedBy = getOntModel().getProperty(OntologyUtils.NS + "is_affected_by");
		Property hasMedicalFactor = getOntModel().getProperty(OntologyUtils.NS + "has_medical_factor");
		Property hasDiseaseId = getOntModel().getProperty(OntologyUtils.NS + "has_disease_id");
		Property hasMedicalFactorId = getOntModel().getProperty(OntologyUtils.NS + "has_medical_factor_id");
		...
		if (results.hasNext()) {
			...
			// medical condition
			RDFNode hasMedicalConditionValue = remedyResource.getPropertyValue(hasMedicalCondition);
			Individual medicalConditionResource = getOntModel().getResource(hasMedicalConditionValue.toString()).as(
					Individual.class);
			RDFNode hasMedicalConditionIdValue = medicalConditionResource.getPropertyValue(hasMedicalConditionId);
			RDFNode hasMinAgeValue = medicalConditionResource.getPropertyValue(hasMedicalConditionMinAge);
			...
			String medicalConditionName = medicalConditionResource.getURI().substring(OntologyUtils.NS.length())
					.replaceAll("_", " ");
			...
			NodeIterator itt = medicalConditionResource.listPropertyValues(isAffectedBy);
			List<DiseaseEntity> medicalConditionIsAffectedBy = new ArrayList<>();
			while (itt.hasNext()) {
				RDFNode prop = itt.next();
				Individual diseaseRes = getOntModel().getResource(prop.toString()).as(Individual.class);
				DiseaseEntity disease = new DiseaseEntity();
				disease.setDiseaseURI(diseaseRes.toString());
				disease.setDiseaseName(diseaseRes.getURI().substring(OntologyUtils.NS.length()).replaceAll("_", " "));
				disease.setDiseaseId(Long.valueOf(diseaseRes.getPropertyValue(hasDiseaseId).toString()));
				medicalConditionIsAffectedBy.add(disease);
			}
			medicalCondition.setDiseases(medicalConditionIsAffectedBy);

			itt = medicalConditionResource.listPropertyValues(hasMedicalFactor);
			List<MedicalFactorEntity> medicalFactors = new ArrayList<>();
			while (itt.hasNext()) {
				RDFNode prop = itt.next();
				Individual medicalFactorRes = getOntModel().getResource(prop.toString()).as(Individual.class);
				MedicalFactorEntity medicalFactor = new MedicalFactorEntity();
				medicalFactor.setMedicalFactorURI(medicalFactorRes.toString());
				medicalFactor.setMedicalFactorName(medicalFactorRes.getURI().substring(OntologyUtils.NS.length())
						.replaceAll("_", " "));
				medicalFactor.setMedicalFactorId(Long.valueOf(medicalFactorRes.getPropertyValue(hasMedicalFactorId)
						.toString()));
				medicalFactors.add(medicalFactor);
			}
			...
			while (it.hasNext()) {
				...
				Individual adjuvantResource = getOntModel().getResource(property.toString()).as(Individual.class);
				RDFNode hasAdjuvantIdValue = adjuvantResource.getPropertyValue(hasAdjuvantId);
				RemedyPropertyEntity adjuvant = new RemedyPropertyEntity();
				adjuvant.setName(property.toString().substring(OntologyUtils.NS.length()).replaceAll("_", " "));
			...
			while (it.hasNext()) {
				RDFNode property = it.next();
				remedy.addPlantPartUsage(property.toString().substring(OntologyUtils.NS.length()).replaceAll("_", " "));
			}
			it = remedyResource.listPropertyValues(hasTherapeuticalUsage);
			while (it.hasNext()) {
				RDFNode property = it.next();
				Individual therapeuticalResource = getOntModel().getResource(property.toString()).as(Individual.class);
				RDFNode hasTherapeuticalIdValue = therapeuticalResource.getPropertyValue(hasTherapeuticalId);
				RemedyPropertyEntity therapeutic = new RemedyPropertyEntity();
				therapeutic.setName(property.toString().substring(OntologyUtils.NS.length()).replaceAll("_", " "));
			...
			while (it.hasNext()) {
				RDFNode property = it.next();
				Individual frequentUsageResource = getOntModel().getResource(property.toString()).as(Individual.class);
				RDFNode hasFrequentIdValue;
				if (frequentUsageResource.hasProperty(hasAdjuvantId))
					hasFrequentIdValue = frequentUsageResource.getPropertyValue(hasAdjuvantId);
				else
					hasFrequentIdValue = frequentUsageResource.getPropertyValue(hasTherapeuticalId);
				RemedyPropertyEntity frequentUsage = new RemedyPropertyEntity();
				frequentUsage.setName(property.toString().substring(OntologyUtils.NS.length()).replaceAll("_", " "));
	...
	public JsonArray getQueryResults(ArrayList<String> adjuvantEffects, ArrayList<String> therapeuticalEffects,
			MedicalConditionEntity medicalCondition) {
		String sparqlQueryString = OntologyUtils.SPARQL_PREFIXES + "SELECT distinct ?subject "
		...
		for (String adjuvant : adjuvantEffects) {
			...
			String clas = "?class" + i;
			sparqlQueryString += "{ ?subject sedic:hasAdjuvantUsage " + individual + " . " + individual + " a  " + clas
		...
		for (String th : therapeuticalEffects) {
			String individual = "?ind" + i;
			String clas = "?class" + i;
			sparqlQueryString += "{ ?subject sedic:hasTherapeuticalUsage " + individual + " . " + individual + " a  "
					+ clas + " . " + clas + " rdfs:subClassOf* " + th + " . } UNION { "
		...
		while (results.hasNext()) {
			...
			remedyEntity.setRemedyPlantId(Long.valueOf(hasPlantIdValue.toString()));
